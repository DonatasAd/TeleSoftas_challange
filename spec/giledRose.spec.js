const { Shop, Item, DecoratedItem } = require('../src/gildedRose');
...
describe('spec/giledRose.spec.js', () => {
  ...
  it('Should lower both values for every item', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('foo', 10, 10)), new DecoratedItem(new Item('bar', 10, 10))]);
    const items = gildedRose.updateQuality();
    expect(items[0].getItemQuality()).toEqual(9);
    expect(items[0].getItemSellIn()).toEqual(9);
    expect(items[1].getItemQuality()).toEqual(9);
    expect(items[1].getItemSellIn()).toEqual(9);
  });
  ...
  it('Should degrade quality twice as fast after sell date has passed', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('foo', 0, 2))]);
    const items = gildedRose.updateQuality();
  ...
  it('Should stop degrade quality at 0', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('foo', 0, 0))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
    expect(quality).toEqual(0);
  });
  ...
  it('Should increase "Aged Brie" quality older it gets', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('Aged Brie', 1, 0))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
  ...
  it('Should stop increasing quality at 50', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('Aged Brie', 1, 50))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
  ...
  it('Should not decrease "Sulfuras" quality', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('Sulfuras, Hand of Ragnaros', 0, 80))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
  ...
  it('Should increase "Backstage passes" quality', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('Backstage passes to a TAFKAL80ETC concert', 20, 49))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
    expect(quality).toEqual(50);
  });
  ...
  it('Should increase "Backstage passes" quality by 2 when 10 days left', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('Backstage passes to a TAFKAL80ETC concert', 10, 20))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
    expect(quality).toEqual(22);
  });
  it('Should increase "Backstage passes" quality by 2 when 6 day left', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('Backstage passes to a TAFKAL80ETC concert', 6, 20))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
    expect(quality).toEqual(22);
  });
  ...
  it('Should increase "Backstage passes" quality by 3 when 5 days left', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('Backstage passes to a TAFKAL80ETC concert', 5, 20))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
    expect(quality).toEqual(23);
  });
  it('Should increase "Backstage passes" quality by 3 when 1 days left', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('Backstage passes to a TAFKAL80ETC concert', 1, 20))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
    expect(quality).toEqual(23);
  });
  ...
  it('Should drop "Backstage passes" quality to 0', () => {
    const gildedRose = new Shop([new DecoratedItem(new Item('Backstage passes to a TAFKAL80ETC concert', 0, 20))]);
    const items = gildedRose.updateQuality();
    const quality = items[0].getItemQuality();
    expect(quality).toEqual(0);
  });
